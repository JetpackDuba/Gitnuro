...
- **Rust:** Gitnuro is mainly written in Kotlin (JVM) but also uses Rust for some specific tasks. To set up your Rust
  environment, please read [its documentation](https://www.rust-lang.org/). `cargo` and `rustc` must be available in
  the path in order to build Gitnuro properly.

  Gitnuro's Rust code uses the toolchain declared in `rs/rust-toolchain.toml`, which currently requires **nightly**.
  Install it with:
  ...
  If you use Homebrew on macOS, make sure the `rustup` cargo is used instead of Homebrew's standalone cargo. A working
  PATH is:
  ...
  ```bash
  which cargo
  cd rs && rustup show active-toolchain
  ```

  `which cargo` should point to the rustup-managed cargo, and the active toolchain inside `rs/` should be `nightly`.
- **cargo-kotars:** A tool to run autogenerated bindings from Rust to Kotlin. Install it with:

  ```bash
  cargo install cargo-kotars --git https://github.com/JetpackDuba/kotars
  ```

  Make sure `cargo-kotars` is in your path, otherwise Gitnuro's build will fail. You can verify it with:

  ```bash
  which cargo-kotars
  cargo-kotars --version
  ```
