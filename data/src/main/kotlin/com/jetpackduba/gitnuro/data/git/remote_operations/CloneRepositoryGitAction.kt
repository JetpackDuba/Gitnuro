...
class CloneRepositoryGitAction @Inject constructor(
    ...
    override operator fun invoke(directory: File, url: String, cloneSubmodules: Boolean): Flow<CloneState> =
        callbackFlow {
            ...
            try {
                ...
                val result = handleTransportGitAction(null) {
                    Git.cloneRepository()
                ...
                when (result) {
                    is Either.Err -> {
                        trySend(CloneState.Fail(result.error))
                    }

                    is Either.Ok -> {
                        val git = result.value
                ...
                if (ex.cause?.cause is CancellationException) {
                    ...
                    trySend(CloneState.Fail(GenericError(ex.localizedMessage)))
                }
            } finally {
                channel.close()
            }
